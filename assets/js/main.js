...
const ui = {
  ...
  songs: { tab: 'unheard', q: '', brands: new Set(), scales: null, events: null, sort: 'recent', includeExtra: false, neverBrand: '', costar: new Set() },
...
/** noPrev chipのポップアップ */
function showNoPrevPopup(anchor, title, livesText) {
  // 既存ポップアップを削除
  document.querySelectorAll('.noprev-popup').forEach(el => el.remove());
  ...
  const linesHtml = livesText
    ? livesText.split('\n').map(l => `<div style="font-size:11px;opacity:.75;margin-top:2px">${esc(l)}</div>`).join('')
    : '<div style="font-size:11px;opacity:.6">（ライブ情報なし）</div>';

  pop.innerHTML =
    `<div style="font-weight:bold;margin-bottom:4px">${esc(title)}</div>` +
    `<div style="font-size:11px;opacity:.6;margin-bottom:4px">立ち会ったライブ：</div>` +
    linesHtml;
  ...
  const pw = 268, ph = pop.offsetHeight || 120;
...
function liveSheet(liveId){
  ...
  openSheet(`
    ...
    ${rows.length ? `
      ...
      ${rows.map(r => {
        ...
        const perfOv = att?.perfOverride?.[r.key];
        const dispPerf = perfOv !== undefined ? (perfOv || []) : r.performers;
        return `<tr>
          <td class="n">${r.seq}</td>
          <td>${esc(DB.song[r.song_id]?.title || r.song_id)}${r.stage_type ? ` <span class="badge part">${esc(perfLabel(r.stage_type))}</span>` : ''}${r.version ? ` <span class="badge none">${esc(S.VERSION_LABEL[r.version] || r.version)}</span>` : ''}
            <div style="margin-top:3px">${dispPerf.map(p =>
              `<span class="dot" style="--c:${DB.idol[p]?.color};display:inline-block;margin-right:2px"></span>`).join('')}
              <span style="font-size:10.5px;color:var(--ink-soft)"> ${esc(dispPerf.map(p => DB.idol[p]?.name || p).join('・') || '—')}</span>
              ${perfOv !== undefined ? '<span class="badge part" style="margin-left:4px;font-size:9px">✎編集済</span>' : ''}
            </div></td>
          <td style="text-align:right;white-space:nowrap">
            <button class="badge ${cls}" data-row="${esc(r.key)}"
              style="background:none;cursor:pointer">${txt}</button>
            <button class="badge none" data-perf-edit="${esc(r.key)}"
              style="background:none;cursor:pointer;margin-left:4px;font-size:9px" title="出演者を編集">✎</button>
          </td>
        </tr>`;
      }).join('')}
  ...
  `, root => {
    ...
    // 出演者編集ボタン
    root.querySelectorAll('[data-perf-edit]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const key = btn.dataset.perfEdit;
        const row = DB.rows.find(r => r.key === key);
        if(!row) return;
        showPerfEditSheet(liveId, row);
      });
    });
  });
}
...
/** 出演者編集シート */
function showPerfEditSheet(liveId, row) {
  const att = S.get().attendance[liveId];
  const perfOv = att?.perfOverride?.[row.key];
  // 現在の出演者（DB or 上書き）をベースに、ライブ全体の出演者候補を列挙
  const allPerfInLive = [...new Set((DB.rowsByLive[liveId] || []).flatMap(r => r.performers))];
  // 候補が少ない場合はDB曲のoriginal_membersも追加
  const baseCandidates = new Set([...allPerfInLive, ...(row.performers || [])]);
  const candidates = [...baseCandidates].filter(id => DB.idol[id]);
  // ブランドでグループ化
  const byBrand = new Map();
  candidates.forEach(id => {
    const idol = DB.idol[id];
    const b = idol.brand_id;
    if(!byBrand.has(b)) byBrand.set(b, []);
    byBrand.get(b).push(id);
  });

  // 現在選択中
  const current = new Set(perfOv !== undefined ? (perfOv || []) : row.performers);

  const renderIdols = (sel) => {
    return [...byBrand.entries()].map(([brandId, ids]) => {
      const brand = DB.brand?.[brandId] || {};
      return `<div style="margin-bottom:8px">
        <div style="font-size:10px;opacity:.6;margin-bottom:4px">${esc(brand.name_short || brandId)}</div>
  ...
  const song = DB.song[row.song_id];
  openSheet(`
    <h3>出演者を編集</h3>
    <p class="sub">${esc(song?.title || row.song_id)} — #${row.seq}</p>
    <p class="sub" style="margin-bottom:10px">タップで出演・非出演を切り替えます。DBのデータと異なる場合（一部歌唱・途中合流など）に使ってください。</p>
    <div id="perf-edit-list">${renderIdols(current)}</div>
    <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn" id="perf-save">保存</button>
      ${perfOv !== undefined ? '<button class="btn ghost" id="perf-reset">DBの情報に戻す</button>' : ''}
      <button class="btn ghost wide" data-close>キャンセル</button>
    </div>
  `, root => {
    const sel = new Set(current);

    root.querySelectorAll('.perf-toggle').forEach(b => {
      b.addEventListener('click', () => {
        ...
        const idol = DB.idol[id];
        if(sel.has(id)){
          b.style.background = idol?.color || 'var(--ink)';
          b.style.color = '#fff';
    ...
    root.querySelector('#perf-save').addEventListener('click', () => {
      S.setPerfOverride(liveId, row.key, [...sel]);
      closeSheet();
      const scrollBack = () => {
        const sheetIn = sheetEl?.querySelector('.sheet-in');
        const top = sheetIn?.scrollTop || 0;
        liveSheet(liveId);
        requestAnimationFrame(() => {
          const s2 = sheetEl?.querySelector('.sheet-in');
    ...
    const resetBtn = root.querySelector('#perf-reset');
    if(resetBtn) resetBtn.addEventListener('click', () => {
      S.setPerfOverride(liveId, row.key, null);
      closeSheet();
      liveSheet(liveId);
      toast('DBの情報に戻しました');
    });
...
/** 共演絞り込みのアイドルピッカーポップアップ */
function showCostarPicker(anchor) {
  // 既存ポップアップを削除
  document.querySelectorAll('.costar-picker').forEach(el => el.remove());

  const scope = S.get().settings.scope;
  const st = compute(scope);
  // 絞り込み対象ブランド（楽曲DBのブランドフィルタに合わせる）
  const activeBrand = ui.songs.brands.size === 1 ? [...ui.songs.brands][0] : null;

  // ブランドでグループ化したアイドルリスト（出演記録のあるものを優先表示）
  const heardIdols = new Set(st.rows.flatMap(r => r.performers));
  const brandsToShow = activeBrand
    ? DB.brands.filter(b => b.brand_id === activeBrand)
    : DB.brands;
  ...
  // ブランドタブ
  let currentBrand = activeBrand || DB.brands[0]?.brand_id;

  const renderPicker = () => {
    const brand = DB.brand?.[currentBrand];
    const idols = DB.idols.filter(i =>
      i.brand_id === currentBrand && i.is_active && !ui.songs.costar.has(i.idol_id)
    );
    pop.innerHTML = `
      <div style="font-weight:bold;margin-bottom:8px">共演アイドルを選ぶ</div>
      <div class="chips" style="flex-wrap:wrap;gap:4px;margin-bottom:10px">
        ${brandsToShow.map(b =>
          `<button class="chip mini picker-brand-tab" type="button"
            data-brand="${b.brand_id}"
            style="border-color:${b.color_primary};${b.brand_id === currentBrand
              ? 'background:' + b.color_primary + ';color:#fff;font-weight:bold'
              : ''}">${esc(b.short_name)}</button>`
        ).join('')}
      </div>
      <div class="chips" style="flex-wrap:wrap;gap:4px">
        ${idols.length
          ...
          : '<span style="font-size:11px;opacity:.5">選択済み or 該当なし</span>'
        }
      </div>
    `;

    pop.querySelectorAll('.picker-brand-tab').forEach(b => {
      b.addEventListener('click', e => { e.stopPropagation(); currentBrand = b.dataset.brand; renderPicker(); });
    });
    pop.querySelectorAll('.picker-idol').forEach(b => {
      b.addEventListener('click', e => {
        e.stopPropagation();
        ui.songs.costar.add(b.dataset.idol);
        pop.remove();
  ...
  pop.style.left = Math.max(8, left) + 'px';
  pop.style.top = Math.max(8, Math.min(top, window.innerHeight - 320)) + 'px';
...
function viewStats(){
  ...
  // 全曲レア度（全件、viewStats内で再利用）
  const _allRare = globalRareSongs(st);
  const _noPrevCount = _allRare.filter(x => x.gapDays < 0).length;
  ...
  <section class="sec">
    ...
    <div class="tallies">
      ${[
        ...
        [_noPrevCount, '一期一会初体験', 'DB上で直前の披露記録がない曲と初めて出会った数', ''],
  ...
  ${(() => {
    const allRare = _allRare;   // viewStats冒頭で計算済み
    ...
    if (noPrev.length) {
      html += '<h3 class="sec-h" style="margin-top:18px;font-size:13px">DB収録以来初披露に立ち会った曲</h3>' +
        ...
        noPrev.map(x => {
          const liveLabel = (() => {
            const rows = DB.rowsBySong[x.sid] || [];
            const myLives = rows
              .filter(r => st.liveIds.has(r.live_id))
              .map(r => DB.live[r.live_id])
              .filter(Boolean);
            const seen = new Set();
            return myLives
              .filter(l => { if(seen.has(l.live_id)) return false; seen.add(l.live_id); return true; })
              .sort((a,b) => a.date.localeCompare(b.date))
              .map(l => {
                const t = DB.tour?.[l.tour_id];
                return fmtDateJP(l.date) + ' ' + (t?.name_short || l.title || l.live_id);
              }).join('\n');
          })();
          return '<button class="chip mini noprev-chip" type="button"'
            + ' style="border-color:' + brandOf(x.song?.brand_id)?.color_primary + ';cursor:pointer;background:none"'
            + ' data-noprev="' + esc(x.song?.title || x.sid) + '"'
            + ' data-noprev-lives="' + esc(liveLabel) + '">'
            + esc(x.song?.title || x.sid)
            + ' <span style="font-size:9px;opacity:.7">' + esc(brandOf(x.song?.brand_id)?.short_name || '') + '</span></button>';
        }).join('') + '</div>';
    }
  ...
  // noPrev chip: クリックで「どのライブで立ち会ったか」ポップアップ
  view.querySelectorAll('.noprev-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      const title = btn.dataset.noprev;
      const lives = btn.dataset.noprevLives;
      showNoPrevPopup(btn, title, lives);
    });
  });
}
const SCOPES_LABEL = () => {
...
function viewSongs(){
  ...
  if(f.tab === 'never'){
    ...
    if(f.costar.size) {
      const costarIds = [...f.costar];
      const songIds = new Set(DB.rows.filter(r =>
        costarIds.every(id => r.performers.includes(id))
      ).map(r => r.song_id));
      list = list.filter(x => songIds.has(x.song.song_id));
    }
    list = sortList(list);
  ...
  <section class="sec">
    ...
    ${(f.tab !== 'all' && f.tab !== 'unheard') ? '' : (() => {
      // 共演絞り込みUI: 選択中のアイドルをchipで表示 + 追加ボタン
      ...
      return `<div class="chips" style="margin-bottom:10px;flex-wrap:wrap;gap:4px;align-items:center">
        <span class="note" style="align-self:center;margin-right:2px;font-size:11px">共演</span>
        ${f.costar.size ? costarChips : '<span style="font-size:11px;opacity:.5">絞り込みなし</span>'}
        <button class="chip mini" type="button" id="costar-add-btn">＋ アイドルを追加</button>
        ${f.costar.size ? '<button class="chip mini" type="button" id="costar-clear">クリア</button>' : ''}
      </div>`;
    })()}
  ...
  // 共演: 選択中アイドル除去
  view.querySelectorAll('[data-costar-remove]').forEach(b => b.addEventListener('click', () => {
    ui.songs.costar.delete(b.dataset.costarRemove);
    renderKeepScroll(viewSongs);
  }));

  // 共演: クリア
  view.querySelector('#costar-clear')?.addEventListener('click', () => {
    ui.songs.costar.clear();
    renderKeepScroll(viewSongs);
  });

  // 共演: アイドル追加ポップアップ
  view.querySelector('#costar-add-btn')?.addEventListener('click', e => {
...
function viewHeatmap(){
  ...
  // ===== アイドル統計計算 =====
  // このアイドルが歌った曲 (過去全ライブ)
  const idolSungSongs = new Set();
  DB.rows.forEach(r => {
    if(r.performers.some(p => selectedIds.has(p))) idolSungSongs.add(r.song_id);
  });

  // ブランド内の全曲
  const brandSongs = DB.songs.filter(s => s.brand_id === activeBrand && !isExtra(s));
  const brandSongIds = new Set(brandSongs.map(s => s.song_id));

  // ライブで一度でも披露された曲（ブランド内）
  const liveSungInBrand = new Set(
    DB.rows.filter(r => brandSongIds.has(r.song_id)).map(r => r.song_id)
  );

  // 分類
  // A: アイドルが歌唱 → idolSungSongs
  // B: ライブ披露あり・アイドルは未歌唱 → liveSungInBrand - idolSungSongs
  // C: ライブ未披露（= brandSongIds - liveSungInBrand）
  const sungByIdol = brandSongs.filter(s => idolSungSongs.has(s.song_id));
  const liveButNotIdol = brandSongs.filter(s => liveSungInBrand.has(s.song_id) && !idolSungSongs.has(s.song_id));
  const neverLive = brandSongs.filter(s => !liveSungInBrand.has(s.song_id));

  // 各曲の最終披露情報
  const lastSungByIdol = song => {
    const rows = (DB.rowsBySong[song.song_id] || [])
      .filter(r => r.performers.some(p => selectedIds.has(p)))
      .map(r => DB.live[r.live_id]).filter(Boolean)
      .sort((a, b) => b.date.localeCompare(a.date));
    return rows[0] || null;
  };
  const lastSungAnywhere = song => {
    const rows = (DB.rowsBySong[song.song_id] || [])
      .map(r => DB.live[r.live_id]).filter(Boolean)
      .sort((a, b) => b.date.localeCompare(a.date));
    return rows[0] || null;
  ...
  // 歌唱回数
  const idolSungCount = song => (DB.rowsBySong[song.song_id] || [])
    .filter(r => r.performers.some(p => selectedIds.has(p))).length;
  const totalSungCount = song => (DB.rowsBySong[song.song_id] || []).length;

  const today = new Date().toISOString().slice(0, 10);
  const gapDays = date => date
    ? Math.round((Date.parse(today) - Date.parse(date)) / 86400000)
    : null;

  const fmtGapShort = d => d === null ? '—' : d === 0 ? '今日' : `${d}日前`;
  const fmtGapLong  = d => d === null ? '記録なし' : d < 30 ? `${d}日前` : d < 365
    ? `${Math.floor(d/30)}ヶ月前（${d}日前）`
    : `${Math.floor(d/365)}年${Math.floor((d%365)/30)}ヶ月前（${d}日前）`;

  // 統計サマリー
  const statSummary = `
    <div class="tiles" style="margin:12px 0 16px">
      <div class="tile"><b>${idolParticipatedLives.size}</b><span>出演公演数</span></div>
      <div class="tile"><b>${sungByIdol.length}</b><span>歌唱楽曲数</span></div>
      <div class="tile"><b>${liveButNotIdol.length}</b><span>未歌唱曲<small>（他では披露済）</small></span></div>
      <div class="tile"><b>${neverLive.length}</b><span>ライブ未披露曲</span></div>
    </div>`;

  // 歌唱曲テーブル（最終歌唱が古い順）
  const sungSorted = [...sungByIdol].sort((a, b) => {
    const la = lastSungByIdol(a), lb = lastSungByIdol(b);
    if(!la && !lb) return 0;
    if(!la) return 1; if(!lb) return -1;
    return la.date.localeCompare(lb.date); // 古い順
  });

  const songTableA = sungSorted.length ? `
    <details open>
      <summary class="sec-h" style="cursor:pointer;font-size:13px;margin-bottom:6px">
        ▸ 歌唱楽曲（${sungByIdol.length}曲）
      </summary>
      <p class="sec-note" style="margin-bottom:6px">最後に歌った日が古い順。回数が少ない = 今後レアかも。</p>
      <table class="lst"><thead><tr>
        <th>楽曲</th><th>最終歌唱</th><th style="text-align:right">歌唱回数</th>
      </tr></thead><tbody>
      ${sungSorted.map(s => {
        const last = lastSungByIdol(s);
        const cnt = idolSungCount(s);
        const gap = gapDays(last?.date);
        return `<tr>
          <td>${esc(s.title)}
            ${s.original_members.some(id => selectedIds.has(id))
              ? '<span class="badge orig" style="margin-left:4px">オリメン</span>' : ''}
          </td>
          <td class="n" style="font-size:10.5px">${last ? fmtGapLong(gap) + '<br><span style="opacity:.6">' + esc(last.date) + '</span>' : '—'}</td>
          <td style="text-align:right;font-size:12px"><b>${cnt}</b>回</td>
        </tr>`;
      }).join('')}
      </tbody></table>
    </details>` : '';

  // ライブ披露あり・アイドル未歌唱テーブル（最終披露が最近のもの順）
  const notIdolSorted = [...liveButNotIdol].sort((a, b) => {
    const la = lastSungAnywhere(a), lb = lastSungAnywhere(b);
    if(!la && !lb) return 0;
    if(!la) return 1; if(!lb) return -1;
    return lb.date.localeCompare(la.date); // 新しい順
  });

  const songTableB = notIdolSorted.length ? `
    <details>
      <summary class="sec-h" style="cursor:pointer;font-size:13px;margin-bottom:6px">
        ▸ 他では披露済・このアイドルは未歌唱（${liveButNotIdol.length}曲）
      </summary>
      <p class="sec-note" style="margin-bottom:6px">他のキャストが歌っているが、選択アイドルの歌唱記録がない曲。</p>
      <table class="lst"><thead><tr>
        <th>楽曲</th><th>直近披露日</th><th style="text-align:right">全体回数</th>
      </tr></thead><tbody>
      ${notIdolSorted.slice(0, 200).map(s => {
        const last = lastSungAnywhere(s);
        const cnt = totalSungCount(s);
        const gap = gapDays(last?.date);
        return `<tr>
          <td>${esc(s.title)}
            ${s.original_members.some(id => selectedIds.has(id))
          ...
          <td class="n" style="font-size:10.5px">${last ? fmtGapLong(gap) + '<br><span style="opacity:.6">' + esc(last.date) + '</span>' : '—'}</td>
          <td style="text-align:right;font-size:12px">${cnt}回</td>
        </tr>`;
      }).join('')}
      </tbody></table>
      ${notIdolSorted.length > 200 ? `<p class="note">（先頭200件）</p>` : ''}
    </details>` : '';

  // ライブ未披露テーブル
  const songTableC = neverLive.length ? `
    <details>
      <summary class="sec-h" style="cursor:pointer;font-size:13px;margin-bottom:6px">
        ▸ ライブ未披露曲（${neverLive.length}曲）
      </summary>
      <p class="sec-note" style="margin-bottom:6px">DBに収録されているが、どのライブでも披露記録がない曲。</p>
      <div class="chips" style="flex-wrap:wrap;gap:4px">
        ${neverLive.map(s => {
          const isOrig = s.original_members.some(id => selectedIds.has(id));
          return `<span class="chip mini"
            style="border-color:${brandOf(s.brand_id)?.color_primary};${isOrig ? 'background:' + brandOf(s.brand_id)?.color_primary + ';color:#fff' : ''}">
            ${esc(s.title)}${isOrig ? ' ★' : ''}
          </span>`;
        }).join('')}
      </div>
  ...
  <section class="sec">
    ...
    <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
      <button class="chip mini" type="button" id="hm-tab-stat" aria-pressed="true">📊 楽曲統計</button>
      <button class="chip mini" type="button" id="hm-tab-matrix" aria-pressed="false">🗓 マトリクス表</button>
    </div>

    <div id="hm-panel-stat">
      ${songTableA}
      ${songTableB ? '<div style="margin-top:14px">' + songTableB + '</div>' : ''}
      ${songTableC ? '<div style="margin-top:14px">' + songTableC + '</div>' : ''}
    </div>

    <div id="hm-panel-matrix" hidden>
...
function _wireHeatmap(){
  ...
  // 統計 / マトリクス タブ切り替え
  const panelStat   = view.querySelector('#hm-panel-stat');
  const panelMatrix = view.querySelector('#hm-panel-matrix');
  view.querySelector('#hm-tab-stat')?.addEventListener('click', () => {
    panelStat && (panelStat.hidden = false);
    panelMatrix && (panelMatrix.hidden = true);
    view.querySelector('#hm-tab-stat')?.setAttribute('aria-pressed','true');
    view.querySelector('#hm-tab-matrix')?.setAttribute('aria-pressed','false');
  });
  view.querySelector('#hm-tab-matrix')?.addEventListener('click', () => {
    panelStat && (panelStat.hidden = true);
    panelMatrix && (panelMatrix.hidden = false);
    view.querySelector('#hm-tab-stat')?.setAttribute('aria-pressed','false');
    view.querySelector('#hm-tab-matrix')?.setAttribute('aria-pressed','true');
  });
}
