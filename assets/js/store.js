...
const DEFAULTS = () => ({
  ...
  attendance: {},        // live_id -> { mode, seat, memo, perfOverride: {rowKey->idol_id[]|null} }
  tiers: {},             // live_id -> 'S'|'A'|…|'F'
  origOverride: {},      // "<live_id>#<seq>" -> true | false
  settings: {
    scope: 'onsite',           // onsite | onsite_lv | all
    stages: ['cast', 'xr'],    // 回収に数えるライブ形式: cast | xr（デフォルト: 全部）
...
/** 半券の出演者上書き。rowKey="<live_id>#<seq>", idolIds=null で削除 */
export function setPerfOverride(liveId, rowKey, idolIds){
  const att = state.attendance[liveId];
  if(!att) return;
  if(!att.perfOverride) att.perfOverride = {};
  if(idolIds === null) delete att.perfOverride[rowKey];
  else att.perfOverride[rowKey] = idolIds;
  commit();
}
