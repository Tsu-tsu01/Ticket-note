...
export function autoJudge(row){
  ...
  // 周年曲・アニメOP等の全体曲（song_type='all' かつ original_members 未設定）→
  // そのブランドに所属するアイドルが1人でも performers にいればオリメン（FULL）とみなす
  if(song.song_type === 'all' && (!song.original_members || song.original_members.length === 0)){
    const brandIdols = new Set(
      DB.idols.filter(i => i.brand_id === song.brand_id).map(i => i.idol_id)
    );
    const hasAny = row.performers.some(p => brandIdols.has(p));
    return hasAny ? 'FULL' : 'NONE';
  }

  // 通常判定: original_members が設定されている曲
  const orig = song.original_members || [];
  if(!orig.length) return 'UNKNOWN';

  const set = new Set(row.performers);
  // performers に original_members のメンバーが1人でも入っていれば FULL 判定
  // (「自分が歌唱メンバーとして入っている曲を歌った場合はオリメン」仕様)
  const hit = orig.filter(id => set.has(id)).length;
  if(hit === 0) return 'NONE';
  // オリメン全員揃っているかに関わらず、1人でも入っていれば歌唱者目線でのオリメン
  // → hit > 0 なら FULL として返す（歌唱メンバー視点での判定）
  return 'FULL';
}
