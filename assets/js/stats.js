...
export function compute(scopeKey){
  ...
  const origByBrand = DB.brands.map(b => {
    // 分母: そのブランドの全楽曲（リミックス・カバー含む）
    const allSongs = DB.songs.filter(s => s.brand_id === b.brand_id);
    const done = allSongs.filter(s => origSeen.has(s.song_id));
    return { brand: b, done: done.length, total: allSongs.length };
  }).filter(x => x.total > 0);
...
function prevPerformDate(songId, firstDate) {
  const rows = DB.rowsBySong[songId] || [];
  let best = null, bestLive = null;
  rows.forEach(r => {
    const live = DB.live[r.live_id];
    const d = live?.date;
    if (!d || d >= firstDate) return;   // 自分が聴いた当日以降は除外
    if (!best || d > best) { best = d; bestLive = live; }
  });
  return { date: best, live: bestLive };
}
 ...
 */
export function globalRareSongs(st, n = Infinity) {
  const result = [];
  st.songN.forEach((_, sid) => {
    const s = DB.song[sid];
    ...
    if (!firstDate) return;
    const prevInfo = prevPerformDate(sid, firstDate);
    const prev = prevInfo.date;
    const gapDays = prev
      ? Math.floor((new Date(firstDate) - new Date(prev)) / 86400000)
      : -1;  // 直前披露なし
    result.push({ sid, song: s, firstDate, prevDate: prev, prevLive: prevInfo.live, gapDays });
  });
  return result
    .sort((a, b) => {
  ...
  return DB.brands
    ...
    .map(b => {
      ...
      const withGap = heard
        .map(sid => {
          const firstDate = st.songFirst.get(sid);
          const prevInfo = firstDate ? prevPerformDate(sid, firstDate) : { date: null, live: null };
          const prev = prevInfo.date;
