...
**公演 87 件・セットリスト 約 1,870 行**（765AS 1st〜6th Anniversary・7th・8th・9th・各プロデューサーミーティング・MR ST@GE・初星宴舞・SUNRICH COLORFUL・Dreamin' Groove ほか、2025〜2026年の各ブランドも収録）。
...
「セトリ表」タブでは、アイドルを選ぶと **縦軸 = そのアイドルの全歌唱曲 × 横軸 = ブランド内全ライブ** のマトリクスを表示する。

| マーク | 意味 |
|---|---|
| ★ 赤 | オリメン歌唱（`is_original` が true と判定） |
| ● 黄 | 歌唱（オリメン以外） |
| 薄灰 | 参加したが未披露（そのライブにアイドルは出たが、この曲は歌わなかった） |
| ほぼ透明 | 不参加（そのライブ自体にアイドルが出ていない） |

横軸のライブ列ヘッダーは `ブランド略称 / YYMMDD / Day略` の 3 行で表示する。
どの曲が連続して歌われているか・どの曲は久しく歌われていないか・どのライブは不参加だったかが一目で分かる。

### ブランド別ライブ楽曲回収率

「統計」タブの「ブランド別 ライブ楽曲回収率」パネルでは、**DB のライブ記録で1回以上披露実績のある曲**を分母に、
自分が 1 回以上聴いた曲を分子として回収率を出す。
楽曲 DB 全体（まだ披露されていない新曲を含む）を分母にする「どれだけ聴いたか」パネルと並べて参照できる。
